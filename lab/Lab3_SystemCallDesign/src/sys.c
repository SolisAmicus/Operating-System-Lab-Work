...
#define __prctl_check_order(__m1, __op, __m2) \
	((unsigned long)prctl_map->__m1 __op(unsigned long) prctl_map->__m2) ? 0 : -EINVAL
				...
				unsigned long, arg4, unsigned long, arg5)
{
	...
	case PR_MCE_KILL_GET:
		...
		if (current->flags & PF_MCE_PROCESS)
			error = (current->flags & PF_MCE_EARLY) ? PR_MCE_KILL_EARLY : PR_MCE_KILL_LATE;
		else
...
static void get_process_time(struct task_struct *p, struct ljj_rusage *usage)
{
    u64 utime, stime;
    ...
    usage->ru_utime.tv_sec = utime / NSEC_PER_SEC;
    usage->ru_utime.tv_nsec = utime % NSEC_PER_SEC;

    usage->ru_stime.tv_sec = stime / NSEC_PER_SEC;
    usage->ru_stime.tv_nsec = stime % NSEC_PER_SEC;
}


static void get_page_faults(struct task_struct *p, struct ljj_rusage *usage)
{
	usage->ru_minflt = p->signal->min_flt;
	usage->ru_majflt = p->signal->maj_flt;
}

static void get_swap_count(struct task_struct *p, struct ljj_rusage *usage)
{
	...
	if (mm)
	{
		nswap = get_mm_counter(mm, MM_SWAPENTS);
		mmput(mm);
	}
	usage->ru_nswap = nswap;
}

SYSCALL_DEFINE2(get_process_usage, pid_t, lPID, struct ljj_rusage __user *, lRU)
{
	...
	get_process_time(task, &usage);
	get_page_faults(task, &usage);
	get_swap_count(task, &usage);

	put_task_struct(task);

	ret = copy_to_user(lRU, &usage, sizeof(usage)) ? -EFAULT : 0;
	return ret;
}